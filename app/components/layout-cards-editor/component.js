/* globals _ */
// allow functions w/in a loop - just this once
/*jshint -W083 */
/**
 * card-canvas:component
 *
 * Essentially an Ember-ized version of GridStack.js
 * https://github.com/troolee/gridstack.js/blob/master/src/gridstack.js
 */
...
export default Ember.Component.extend({
  ...
  hasCards: Ember.computed('cards.length', function(){
  ...
  drop(event){
    console.log('LAYOUT CARD EDITOR CAUGHT DROP!!');
    // Ember.debug('DD DROPPED AT event.currentTarget.offsetLeft: ' + event.currentTarget.offsetLeft);
    // Ember.debug('DD DROPPED AT event.currentTarget.offsetTop: ' + event.currentTarget.offsetTop);

    let ip = this.get('eventBus.dropPosition');
    let card = null;
    if(ip){
      Ember.debug('using insertionPoint x:' + ip.x + ' y:' + ip.y);
      card={
        x:ip.x,
        y:ip.y,
        height:2,
      ...
      //we've used this, so clear it out
      this.set('eventBus.dropPosition',null);
    }else{
      Ember.debug('eventBus.dropPosition was null');
    }
    this.addCard(card);
  },
  ...
  updateCanvasHeight() {
    ...
    Ember.debug('Setting the canvas height to ' + newHeight + ' for ' + this.get('elementId'));
